    ...
    if ((_doillegal || _cls in _combinedItems) && !(_cls in OT_allClothing)) then {
        ...
        if (_qty > 0) then {
            //still need to find more items in backpacks, uniforms etc
            {
                ...
                // Bug fix: _sellcls was a copy-paste artifact from fn_sellAll; without _cls here the subcontainer items were never removed but the export still credited the player -> infinite money/dupe.
                {
                    _x params ["_c", "_q"];

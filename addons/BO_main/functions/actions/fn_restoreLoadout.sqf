...
// Lifted out of the if-block so _uniformCls/_uniformItems are visible at
// lines 139/163 below. Inside `if {} then {}` the privates die at the
// closing brace; the later forceAddUniform + forEach references would
// then evaluate as nil and silently drop the uniform + uniform cargo.
private _uniformCls = "";
private _uniformItems = [];
if (_uniform isNotEqualTo []) then {
    _uniformCls = _uniform select 0;

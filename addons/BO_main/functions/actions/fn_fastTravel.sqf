...
OT_FastTravel_MapSingleClickEHId = addMissionEventHandler [
    "MapSingleClick",
    {
        ...
        if !(_handled) then {
            ...
            // Invulnerability for the crew must start BEFORE the 2s
            // window; otherwise a vehicle blown up during the fade can
            // eject the player and still bill them at vehicle-rate.
            private _vehicleNow = vehicle player;
            if (_vehicleNow != player) then {
                { _x allowDamage false } forEach (crew _vehicleNow);
            };

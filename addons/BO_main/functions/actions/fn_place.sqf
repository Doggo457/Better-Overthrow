    ...
    if (modeCancelled
        ...
        if ([getPos player, _typecls] call OT_fnc_canPlace) then {
            ...
            if (_typecls isEqualTo "Base") then {
                // BO: route through OT_dialog_input + OT_inputHandler
                // (same pattern as BO_fnc_renameFOB) instead of the
                // vanilla OT_dialog_name whose Done button action
                // wasn't reaching OT_fnc_onNameDone on the user's
                // setup. Stash the flag in OT_context so the handler
                // can locate it after closeDialog clears the world
                // reference; on Done/Enter, BO_fnc_commitNewFOB does
                // the registration.
                OT_context = modeTarget;
                OT_inputHandler = { [] call BO_fnc_commitNewFOB };
                ["<t align='center'>Enter a name for your new base</t>", "Base"] call OT_fnc_inputDialog;
            };

    ...
    if (_btn isEqualTo 0 && _sx > (safeZoneX + (0.1 * safeZoneW)) && _sx < (safeZoneX + (0.9 * safeZoneW))) then {
        ...
        if (!isNull modeTarget && canBuildHere) then {
            ...
            if (_money < modePrice) then {
                ...
                // Route the debit through OT_fnc_money so the player
                // gets the standard "-$N: Build" notify and the audit
                // log stays consistent. The raw setVariable left no
                // on-screen confirmation of the cost; players often
                // could not tell whether a click had registered, then
                // clicked again and were charged twice. The redundant
                // playSound here matched what OT_fnc_money plays.
                [-modePrice, "Build"] call OT_fnc_money;
                ...
                if (typeOf _created == OT_warehouse) then {
                    // MP race: warehouse "owned" array is shared server state. Route push through server-only helper.
                    [_created] remoteExec ["BO_fnc_registerWarehouse", 2, false];
                };

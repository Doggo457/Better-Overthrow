...
if (!isNil "_civ" && _civ getVariable ["factionrep", false] && !((_cls isKindOf "Land") || (_cls isKindOf "Air") || (_cls isKindOf "Ship"))) then {
    ...
    if (_faction isNotEqualTo "") then {
        ...
        if (_increase > 0) then {
            ...
            // MP race: standing is shared server state. Route the RMW through server-only resolver.
            ["standing", [_faction, _increase]] remoteExec ["BO_fnc_resolveBuy", 2, false];
...
if (OT_interactingWith getVariable ["factionrep", false] && ((_cls isKindOf "Land") || (_cls isKindOf "Air") || (_cls isKindOf "Ship"))) exitWith {
    ...
    if !(_cls in _blueprints) then {
        [-_price] call OT_fnc_money;
        // MP race: blueprints list is shared server state. Route RMW through server-only resolver (idempotent on duplicates).
...
if (_cls isKindOf ["Default", configFile >> "CfgMagazines"]) exitWith {
    if (_cls in OT_allExplosives) then {
        // MP race / double-spend exploit: reschems was RMW'd from a stale local
        // snapshot, letting two concurrent explosive buys clobber each other.
        // Route the debit through the server-only resolver as a delta.
        ["chems", [-(_cost select 3)]] remoteExec ["BO_fnc_resolveBuy", 2, false];
    };
    [-_price] call OT_fnc_money;

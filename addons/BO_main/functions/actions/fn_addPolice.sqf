...
// MP race: route through server-only adjuster instead of client RMW.
// Display uses optimistic local value; server broadcast will reconcile.
private _garrison = (server getVariable [format ["police%1", _town], 0]) + _amt;
["police%1", _town, _amt] remoteExec ["BO_fnc_adjustTownCounter", 2, false];

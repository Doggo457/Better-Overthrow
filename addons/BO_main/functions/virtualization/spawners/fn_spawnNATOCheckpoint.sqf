...
// BO multi-nation: the per-map checkpoint template hardcodes vanilla
// BLU_F static HMG/GMG classes. Replace each StaticWeapon spawned by
// the mapper with the per-map OT_NATO_HMG class so faction swaps
// (RHS / CUP / 3CB) get the right static nests instead of vanilla.
// Position + direction preserved from the template; only the class
// changes.
private _patchedVehs = [];
{
    if (_x isKindOf "StaticWeapon") then {
        private _spos = getPosATL _x;
        private _sdir = getDir _x;
        deleteVehicle _x;
        private _factionStatic = OT_NATO_HMG createVehicle _spos;
        _factionStatic setPosATL _spos;
        _factionStatic setDir _sdir;
        _patchedVehs pushBack _factionStatic;
    } else {
        _patchedVehs pushBack _x;
    };
} forEach _vehs;
_vehs = _patchedVehs;

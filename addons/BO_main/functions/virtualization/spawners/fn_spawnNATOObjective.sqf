...
private _reconLayout = server getVariable [format ["BO_reconLayout_%1", _name], []];

// BO CRITICAL FIX: HVT/APC were previously tagged with the literal
// string "HQ" instead of the base name, so the recon snapshot stored
// them under the wrong key and the short-circuit below skipped this
// block on respawn -- result: HVT never respawned.
//
// Fix: hoist the HVT/APC block above the recon short-circuit and tag
// with _name (not "HQ"). We only run this block when there is no
// snapshot yet (first spawn) -- once the snapshot exists, it round-
// trips the HVT incl. hvt/hvt_id/NOAI flags through BO_fnc_reconRestore.
private _hvtGroups = [];
if (_reconLayout isEqualTo []) then {
    {
        _x params ["_id", "_loc", "_status"];
        if (_loc == _name && _status == "") then {
            private _group = createGroup blufor;
            _hvtGroups pushBack _group;
...
// BO: short-circuit to the recon-restore path if we have a snapshot
// of this base from the previous despawn. The snapshot is set up by
// BO_fnc_reconSnapshot when a reconned base despawns; restoring from
// it instead of re-rolling positions keeps the garrison layout
// stable across visits, which is what makes recon mean something.
if (_reconLayout isNotEqualTo []) exitWith {
    [_posTown, _name, _spawnid, _reconLayout] call BO_fnc_reconRestore;
};
...
private _vehgarrison = server getVariable [format ["vehgarrison%1", _name], []];
{
    ...
    if (_pos isNotEqualTo []) then {
        ...
        _groups pushBack _veh;
        {
            ...
            // BO: was hardcoded "HQ" -- breaks snapshot lookup for non-HQ bases.
            _x setVariable ["garrison", _name, false];
        } forEach (crew _veh);
...
// BO: HVT/APC block was moved to the top of the file (before the
// recon-restore short-circuit) and tagged with _name (not "HQ") so the
// snapshot captures HVTs and round-trips them through reconRestore.
_groups append _hvtGroups;

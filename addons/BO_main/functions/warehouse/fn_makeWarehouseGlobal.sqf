...
// Route the $10000 debit through OT_fnc_money so the player gets the
// standard "-$10000: Warehouse global" notify + sound and the audit
// log stays consistent. _caller runs locally on the action-clicking
// client, so it IS the local player here -- safe to use OT_fnc_money
// which always operates on `player`. The redundant playSound here
// matched what OT_fnc_money plays.
[-10000, "Warehouse global"] call OT_fnc_money;

...
[
    {
        ...
        if (_numplayers > 0) then {
            ...
            if (time >= OT_nextNATOTurn && !_countered) then {
                ...
                // BO (user-locked): passive resource recovery is REMOVED.
                // NATO's ONLY non-admin income is the physical supply
                // line -- hourly cargo plane (+1000 at a held airfield)
                // or cargo helicopter (+200 at a held base), both
                // interceptable in flight (BO_fnc_supplyRun). HAL
                // op refunds return previously-held budget, not income.
                server setVariable ["NATOlastgain", 0, true];

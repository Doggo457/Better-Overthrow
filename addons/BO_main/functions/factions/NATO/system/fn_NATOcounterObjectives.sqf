...
// BO: defensive — sibling NATO system fns re-fetch _diff locally; this one missed
// the pattern. Without it, difficulty scaling relied on scope inheritance from
// fn_factionNATO, fragile to caller refactor.
...

{
    ...
    if !(_name in _abandoned) then {
        ...
        if ((isNull _drone || !alive _drone) && { _resources > 10 }) then {
            ...
            // Lint-found bug: _knownTargets was undefined here -- the
            // FOB-targeting pass errored out of the counter-attack
            // every time. The buffer is the global NATOknownTargets.

...
// BO: caller passes [] so _spend/_resources cannot come from params; locally derived.
// Without these, downstream `server setVariable ["NATOresources", _resources]` writes
// nil and the ledger silently caps at 2000, blocking war-level progression.
private _resources = server getVariable ["NATOresources", 2000];
private _spend = _resources;

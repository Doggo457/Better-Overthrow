...

{
    ...
    if ((_name isNotEqualTo OT_NATO_Unit_Police) && (_name isNotEqualTo OT_NATO_Unit_PoliceCommander)) then {
        // BO multi-nation: re-derive comms-specific role classes (sniper,
        // spotter, AA pair, HVT officer) from the active faction. Without
        // this, fn_spawnNATOObjective.sqf reads stale per-map vanilla
        // defaults from initVar.sqf (e.g. B_Sniper_F on Altis) even when
        // the player selects RHS/CUP/etc. Done in the same CfgVehicles
        // scan that already filters by OT_faction_NATO at line 101.
        private _roleRaw = toLower (getText (_x >> "role"));
        private _lname = toLower _name;
        if (!("recon" in _lname) && !("story" in _lname) && !("_ctrg_" in _lname)) then {
            switch (true) do {
                case (_roleRaw isEqualTo "sniper"): { OT_NATO_Unit_Sniper = _name };
                case (_roleRaw isEqualTo "spotter"): { OT_NATO_Unit_Spotter = _name };
                case (_roleRaw isEqualTo "missilespecialist" && ("_aa_f" in _lname) && !("_aaa_f" in _lname)): { OT_NATO_Unit_AA_spec = _name };
                case (("_aaa_f" in _lname) || ("aa_assistant" in _lname)): { OT_NATO_Unit_AA_ass = _name };
                case (_roleRaw isEqualTo "officer"): { OT_NATO_Unit_HVT = _name };
            };
        };
    ...
    };
    {
        ...
        if !(_name in _abandoned) then {
            ...
            if (_name isEqualTo OT_NATO_HQ) then {
                _garrison = 48;
                // BO multi-nation: derive HQ garrison from per-map
                // faction vars instead of hardcoded BLU_F/BLU_T_F
                // classes. Static AA umbrella from OT_NATO_Vehicles_StaticAAGarrison
                // (which factions populate with their AA emplacements --
                // plus radar/SAM with Jets DLC) + 6 HMG nests from the
                // per-map OT_NATO_HMG class. With RHS/CUP/etc. the HQ
                // now spawns the right faction's gear instead of vanilla
                // NATO Pacific tanks + BLU_F statics.
                private _hqGar = +OT_NATO_Vehicles_StaticAAGarrison;
                for "_i" from 1 to 6 do { _hqGar pushBack OT_NATO_HMG };
                server setVariable [format ["vehgarrison%1", _name], _hqGar, true];
                private _garr = [];
                {

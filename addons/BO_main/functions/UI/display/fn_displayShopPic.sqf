...
if (_price > -1) then {
    ...
    if (_cls isKindOf "CAManBase") exitWith {
        // UID match: the recruit dialog charges UID-aware cost
        // (per-player loadout pricing). Tooltip must mirror that or
        // players see one price and pay another.
    ...
    if (_cls in OT_allSquads) exitWith {
        // UID match: squad cost rolls up per-soldier UID-aware loadout
        // pricing -- tooltip must match the charge made by recruit.
        private _squad = [_cls, getPlayerUID player] call OT_fnc_getSquad;

...
// BO: promoting Generals is gated to host/admin so the Make General
// button only lights up for them, even though all Generals can see
// other players' money + transfer funds (gated by _amgen below).
private _canPromote = (isServer && hasInterface) || ((call BIS_fnc_admin) isEqualTo 2);
...
if (_amgen && _uid != (getPlayerUID player)) then {
    ...
    if (_canPromote && !(_uid in (server getVariable ["generals", []]))) then {

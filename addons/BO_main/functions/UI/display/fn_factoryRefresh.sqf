/*
 * fn_factoryRefresh (BO override of OT_fnc_factoryRefresh)
 *
 * Multi-factory: reads BO_producing / BO_producetime / BO_queue
 * from OT_interactingWith (the factory the player is managing in
 * this dialog session). Falls back to the legacy server-namespace
 * vars only if OT_interactingWith is null (defensive; shouldn't
 * happen via the normal dialog flow which pins it in
 * fn_factoryDialog).
 */
...
private _hasFactory = (!isNull _factory) && {(typeOf _factory) isEqualTo OT_factory};

private _currentCls = if (_hasFactory) then {
    _factory getVariable ["BO_producing", ""]
} else {
    server getVariable ["GEURproducing", ""]
};

private _queue = if (_hasFactory) then {
    _factory getVariable ["BO_queue", []]
} else {
    server getVariable ["factoryQueue", []]
};

private _timespent = if (_hasFactory) then {
    _factory getVariable ["BO_producetime", 0]
} else {
    server getVariable ["GEURproducetime", 0]
};

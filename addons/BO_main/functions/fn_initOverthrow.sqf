...
// BO multi-nation: per-map initVar.sqf hardcodes BLU_F / BLU_T_F LSV
// + MRAP classes into OT_NATO_Vehicles_Convoy / _GroundSupport /
// _PoliceSupport and the scalar OT_NATO_Vehicle_Transport_Light.
// The faction switch above overwrites OT_faction_NATO only, so when
// the mission param ot_enemy_faction is set to RHS / CUP / UK3CB the
// infantry pool swaps but every wheeled support vehicle stays
// vanilla -- visible as Prowlers parked at every NATO FOB after each
// fn_NATOMissionDeployFOB drop. BO_fnc_factionNATOVehicles re-mines
// these arrays from the active faction's CfgVehicles. Runs after
// OT_fnc_initVar so OT_allBLUOffensiveVehicles is populated, and
// before OT_fnc_initVirtualization so the new pools are visible to
// every spawn path. No-op on native vanilla factions.
[] call BO_fnc_factionNATOVehicles;

// BO multi-nation residual fixups. factionNATOVehicles only covers
// Convoy / GroundSupport / PoliceSupport / Transport_Light. The
// helpers below patch the remaining per-map-hardcoded BLU_F classes:
//   - Statics:     OT_NATO_HMG, OT_NATO_Vehicles_StaticAAGarrison
//   - Vehicles2:   HVT, Police, Boat_Small, Transport (truck array)
//   - Air:         AirTransport + Small + Large + CTRGTransport,
//                  AirGarrison, JetGarrison, ReconDrone
//   - Support:     AirSupport + AirSupport_Small, TankSupport, APC
//   - GarrisonTemplates: StaticGarrison_LevelOne/Two/Three
//                        (depends on Statics + Vehicles2 finishing)
//   - Police:      OT_NATO_Unit_Police + _Heavy + _Commander[_Heavy]
//                  + PoliceMedic_Heavy (graceful-degrade for non-Apex
//                  factions). MUST precede OT_fnc_initNATO which
//                  snapshots OT_Loadout_Police from these classes.
// All run synchronously here so the publicVariables propagate before
// OT_fnc_initVirtualization (and OT_fnc_initNATO down below in the
// spawned block) consume the patched pools.
[] call BO_fnc_factionNATOStatics;
[] call BO_fnc_factionNATOVehicles2;
[] call BO_fnc_factionNATOAir;
[] call BO_fnc_factionNATOSupport;
[] call BO_fnc_factionNATOGarrisonTemplates;
// factionNATOInfantry mines the three HAL Day-1 anti-armor / spec-ops
// vars (OT_NATO_Unit_AT, _AT_Heavy, _SF) from the active faction.
// Police scalars are intentionally left at the per-map Apex Gendarmerie
// (B_Gen_*) defaults -- Gendarmerie is DLC-tied not faction-tied, so
// police stations stay police regardless of the military faction param.
// factionNATOPolice is a back-compat shim that calls this.

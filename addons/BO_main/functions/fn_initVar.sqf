...
OT_Buildables = [
    ...
    ["Factory", 20000, [OT_factory], "BO_fnc_initFactory", false, "Production facility -- produces vehicles and weapons from blueprints. A cargo container for material input spawns next to it. Replace the container later via the Y menu if you want to move it."],
    // BO production businesses. Each ticks once per in-game hour via
    // BO_fnc_businessTick: pays wages from resistance funds, consumes
    // input (if any) from a nearby cargo container, deposits output
    // into the business's adjacent I/O crate. Per-object state on the
    // building keeps multiple instances independent (same pattern as
    // multi-factory). See BO_businessTypes for the per-type spec.
    ["Lumberyard",       1500, [OT_workshopBuilding], "BO_fnc_initLumberyard",      false, "Produces Wood. ~10 wood/hr, employs 5. No input required. An I/O cargo container spawns next to it."],
    ["Mine",             5000, [OT_workshopBuilding], "BO_fnc_initMine",            false, "Produces Steel. ~8 steel/hr, employs 6. No input required. An I/O cargo container spawns next to it."],
    ["Vineyard",         2500, [OT_workshopBuilding], "BO_fnc_initVineyard",        false, "Grows Grapes. ~12 grapes/hr, employs 4. No input required. An I/O cargo container spawns next to it."],
    ["Winery",           5000, [OT_workshopBuilding], "BO_fnc_initWinery",          false, "Processes Grapes into Wine. ~8 wine/hr, employs 6. Requires 8 grapes/hr in the adjacent I/O crate."],
    ["Olive Plantation", 2500, [OT_workshopBuilding], "BO_fnc_initOlivePlantation", false, "Produces Olives. ~12 olives/hr, employs 4. No input required. An I/O cargo container spawns next to it."],
    ["Chemical Plant",   8000, [OT_workshopBuilding], "BO_fnc_initChemicalPlant",   false, "Produces Fertilizer. ~6 fertilizer/hr, employs 8. No input required. An I/O cargo container spawns next to it."],
    // BO artillery: a placed B_Mortar_01_F enables the "Call Fire
    // Mission" ACE action. Player picks shell (HE / Smoke / Illum),
    // round count (1/3/6), clicks the map, pays per round from their
    // bank. 5-min cooldown per mortar (mission param).
    ["Mortar Position", 3500, ["B_Mortar_01_F"], "BO_fnc_initMortar", false, "Buildable mortar -- enables 'Call Fire Mission' ACE action. Pay per round from your bank: HE $500, Smoke $150, Illum $100. Cooldown 5 min (mission param)."]
];

// BO production business spec: lookup by display name. Tuple shape:
//   [_inputCls, _outputCls, _outputPerHr, _employees, _placementCost]
// _employees is used for the per-hour wage cost via OT WAGE price.
// _placementCost is informational (the buildable's price slot is the
// real cash gate); it's here so a single source documents the trade.
BO_businessTypes = createHashMap;
BO_businessTypes set ["Lumberyard",       ["",          "OT_Wood",        10,  5,  1500]];
BO_businessTypes set ["Mine",             ["",          "OT_Steel",        8,  6,  5000]];
BO_businessTypes set ["Vineyard",         ["",          "OT_Grapes",      12,  4,  2500]];
BO_businessTypes set ["Winery",           ["OT_Grapes", "OT_Wine",         8,  6,  5000]];
BO_businessTypes set ["Olive Plantation", ["",          "OT_Olives",      12,  4,  2500]];
BO_businessTypes set ["Chemical Plant",   ["",          "OT_Fertilizer",   6,  8,  8000]];

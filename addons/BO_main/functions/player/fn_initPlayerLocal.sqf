...
// MP race: route through server-only registrar so concurrent JIPs
// don't last-write-wins the OT_allplayers roster.
[getPlayerUID player, name player] remoteExec ["BO_fnc_registerPlayerUID", 2, false];
...
// Defensive window: AI/triggers can damage the player before the spawn
// camera preloads and BLACK IN fades. Re-enable after preload + delay.
    ...
    "InventoryOpened",
    {
        ...
        if !(_veh call OT_fnc_playerIsOwner) then {
            ...
            if (!_isgen && (_veh getVariable ["OT_locked", false])) exitWith {
                // name<uid> keys live on players_NS (fn_registerPlayerUID), not server.
                hint format ["This inventory has been locked by %1", players_NS getVariable ("name" + (_veh call OT_fnc_getOwner))];
                _locked = true;
            };
        };
        _locked;
        ...
        if (_position == "driver") then {
            if !(_veh call OT_fnc_playerIsOwner) then {
                ...
                if (!_isgen && (_veh getVariable ["OT_locked", false])) then {
                    moveOut player;
                    hint format ["This vehicle has been locked by %1", players_NS getVariable ("name" + (_veh call OT_fnc_getOwner))];
                };
            };
        } else {
...
if (isClass (configFile >> "CfgPatches" >> "zen_common")) then {
    ...
    // BO testing-helper modules. Drop the module on/near a player to
    // hit them with the dialog; the War Level module is global and
    // ignores placement position.
    //
    // BO: Set Reputation was removed -- it duplicated OT's existing
    // "Change Town Support" module, which already modifies the global
    // server "rep" var (see OT_fnc_support: it writes both rep<town>
    // and rep on every call).
    ["Overthrow", "BO: Set Player Money",       { _this call BO_fnc_zenSetMoneyModule         }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: Set Player Bank",        { _this call BO_fnc_zenSetBankModule          }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: Set War Level",          { _this call BO_fnc_zenSetWarLevelModule      }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: Set NATO Resources",     { _this call BO_fnc_zenSetNATOResourcesModule }] call zen_custom_modules_fnc_register;
    // HAL admin toolkit (server re-validates privileges; restricted
    // curators can't place modules at all -- Place coef -1).
    ["Overthrow", "BO: HAL Status",             { [_this, "status"]   call BO_fnc_zenHalModule }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: HAL Force Tick",         { [_this, "tick"]     call BO_fnc_zenHalModule }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: HAL Clear Active Ops",   { [_this, "clearops"] call BO_fnc_zenHalModule }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: HAL Set Silent Ticks",   { [_this, "silent"]   call BO_fnc_zenHalModule }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: HAL Add Heat Here",      { [_this, "heat"]     call BO_fnc_zenHalModule }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: HAL Set Max Ops",        { [_this, "maxops"]   call BO_fnc_zenHalModule }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: HAL Spawn Package Here", { [_this, "spawnpkg"] call BO_fnc_zenHalModule }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: HAL Doctrine Profile",   { [_this, "doctrine"] call BO_fnc_zenHalModule }] call zen_custom_modules_fnc_register;
    // Phase 2 testing/admin modules.
    ["Overthrow", "BO: Spawn Production Business", { _this call BO_fnc_zenSpawnBusiness        }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: Spawn Factory",          { _this call BO_fnc_zenSpawnFactory           }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: Trigger Demand Event",   { _this call BO_fnc_zenTriggerDemandEvent     }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: Set Town Police",        { _this call BO_fnc_zenSetTownPolice          }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: Set Garrison Level",     { _this call BO_fnc_zenSetGarrison            }] call zen_custom_modules_fnc_register;
    ["Overthrow", "BO: Trigger NATO Counter-attack", { _this call BO_fnc_zenTriggerCounterAttack }] call zen_custom_modules_fnc_register;

    // Right-click context menu: Set Money + Set Bank.
    [] call BO_fnc_initZenContextMenu;
} else {

...
// Drop the 60s timeout: silent failure here loses action wiring on
// objects whenever the local player init takes >60s (slow load, AFK
// in the start dialog, JIP during heavy mission init). Let it wait.
...
// BO: only add the object-level Remove/Confirm to things that DON'T
// already get a class-level one from MACRO_SALVAGEWRECK (applied in
// OT's config.bin to Car/Tank/Motorcycle/Helicopter/Plane/Ship_F).
// Vanilla OT added both, which made every land vehicle show "Remove"
// twice and "Confirm" three times in the ACE wheel.
private _hasClassRemove = (_this isKindOf "LandVehicle")
    || (_this isKindOf "Air")
    || (_this isKindOf "Ship_F");

if (!_hasClassRemove) then {

class OT_sleep_dialog {
    ...
    class controls {
        ...
        class SleepButton: RscButton {
            ...
            h = "0.033 * safeZoneH";
            // Cinematic/cutText/allowDamage must run only on the sleeping client, not broadcast to all players
            onButtonClick = "[] spawn OT_fnc_startSleeping; [] spawn { uiSleep 8; OT_sleepTime remoteExec ['skipTime', 2]; };";
            colorBackground[] = {
                "(profileNamespace getVariable ['GUI_BCG_RGB_R', 0.3843])",
                "(profileNamespace getVariable ['GUI_BCG_RGB_G', 0.7019])",
                "(profileNamespace getVariable ['GUI_BCG_RGB_B', 0.8862])",

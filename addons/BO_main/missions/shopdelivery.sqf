...
//Find a destination
{
    ...
    // BO world demand events: pre-promote any town that has an
    // active event by halving its random sort key so it tends to
    // sort first. Safe with no active events (no key change).
    private _t = _x;
    private _events = server getVariable ["BO_activeWorldEvents", []];
    private _hasEvent = (_events findIf { (_x select 0) isEqualTo _t }) >= 0;
    private _r = random 100;
    if (_hasEvent) then { _r = _r * 0.4 };
    _r

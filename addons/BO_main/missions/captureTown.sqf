            ...
            [
                {
                    ...
                    private _factionName = server getVariable format ["factionname%1", _faction];
                    // Job-flavor message: notify only the player whose
                    // machine is running this PFH (the job's accepter).
                    format ["Incoming message from %1: %2 captured. Donating $2500 to the resistance and will help with efforts to ease the local populace. (+10 stability, +10 %1)", _factionName, _destinationName] call OT_fnc_notifyMinor;
                },

...
[
    ...
    },
    {
        ...
        if (_alerted && !_alreadyAlerted) then {
            _civ enableAI "MOVE";
            format ["NATO Informant has been alerted."] call OT_fnc_notifyMinor;
    ...
    },
    {
        ...
        if (_wassuccess) then {
            // Lint-found bug: the deferred block doesn't capture locals;
            // _destination was nil at execution -> "Informant in any".
            // Pass it through waitAndExecute's args instead.
            [{
                params ["_d"];
                format ["NATO Informant in %1 has been taken care of", _d call OT_fnc_nearestTown] call OT_fnc_notifyMinor;
            }, [_destination], 2] call CBA_fnc_waitAndExecute;
        };
        //Clean up

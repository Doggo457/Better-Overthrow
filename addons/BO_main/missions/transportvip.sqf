...
[
    ...
    },
    {
        ...
        if (_wassuccess) then {
            [
                {
                    params ["_faction"];
                    private _factionName = server getVariable format ["factionname%1", _faction];
                    format ["Incoming message from %1: Thank you for delivering our operative. (+5 %1)", _factionName] call OT_fnc_notifyMinor;
            ...
            private _factionName = server getVariable format ["factionname%1", _faction];
            format ["Incoming message from %1: What happened?!? (-10 %1)", _factionName] call OT_fnc_notifyMinor;

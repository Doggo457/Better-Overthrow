...
[
    ...
    },
    {
        ...
        if (_alerted && !_alreadyAlerted) then {
            _civ enableAI "MOVE";
            private _factionName = server getVariable format ["factionname%1", _faction];
            format ["Incoming message from %1: Traitor has been alerted.", _factionName] call OT_fnc_notifyMinor;
            ...
            [
                {
                    ...
                    private _factionName = server getVariable format ["factionname%1", _faction];
                    format ["Incoming message from %1: Traitor neutralized. Sending our regards and $2500 to the resistance. (+20 %1)", _factionName] call OT_fnc_notifyMinor;
